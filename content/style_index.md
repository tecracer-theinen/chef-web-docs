...
:   The minimum time in milliseconds that Infra Server will keep any given cookbook version response in the cache when
...
not occur if cookbook versions are incremented with every cookbook update, which is the recommended approach to updating cookbooks.
{{< /note >}}
